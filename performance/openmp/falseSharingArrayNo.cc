...
#pragma omp parallel
   ...
   // 8 doubles comprise 64 bytes, the size of a cache line.
   const int Padding = 8;
   ads::Array<1, Number> data(numberOfThreads *(arraySize + Padding));
   ads::Array<1, ads::Array<1, Number, false> > a(numberOfThreads);
   for (int i = 0; i != numberOfThreads; ++i) {
      a[i].rebuild(&data[i *(arraySize + Padding)],
                   &data[i *(arraySize + Padding) + arraySize]);
      a[i] = 0;
   }
