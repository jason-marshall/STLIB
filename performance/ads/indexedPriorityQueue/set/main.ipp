...
main(int argc, char* argv[]) {
   ...
   ads::Array<1, Number> propensities;
   {
   ...
   ads::Array<1, Number> offsets(propensities.size());
   for (std::size_t i = 0; i != offsets.size(); ++i) {
      assert(propensities[i] > 0);
      offsets[i] = 1 / propensities[i];
   }

   const Number sum = ads::computeSum(propensities);
   assert(sum > 0);
   const Number factor = 1 / sum;
   ads::Array<1, Number> initialTimes(propensities.size());
   for (std::size_t i = 0; i != offsets.size(); ++i) {
      initialTimes[i] = i * factor;
   }
   std::random_shuffle(initialTimes.begin(), initialTimes.end());
   ads::Array<1, Number> times(propensities.size());

   // Construct the indexed priority queue.
#ifdef HASHING
   int tableSize = std::max(256, 2 * propensities.size());
   ...
   if (count == 0) {
      ...
      do {
         ...
         // Pop one element so the data structures are initialized.
         indexedPriorityQueue.pushTop(times[indexedPriorityQueue.top()]);

         timer.tic();
         count *= 2;
         for (long i = 0; i != count; ++i) {
            // Change all of the times by shifting.
            const Number first = times[0];
            for (std::size_t n = 0; n != times.size() - 1; ++n) {
               times[n] = times[n + 1];
            }
            *(times.end() - 1) = first;
            ...
            for (std::size_t n = 0; n != times.size(); ++n) {
               indexedPriorityQueue.set(n, times[n]);
            }
         }
   ...
   std::cout << "Meaningless result = " << indexedPriorityQueue.top() << "\n"
             << "Set " << count << " numbers in " << elapsedTime
             << " seconds.\n"
