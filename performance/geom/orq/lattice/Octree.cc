...
main(int argc, char* argv[]) {
   ...
   int leafSize = 8;
   parser.getOption("leafSize", &leafSize);
   assert(leafSize > 0);
   ...
   // Construct the ORQ data structure from the Cartesian
   // domain that contains the records.  Make the domain a little larger
   // than the unit cube to ensure that all of the records are inside.
   Orq orq(SemiOpenInterval(Point(-dx / 2, -dx / 2, -dx / 2),
                            Point(1 + dx / 2, 1 + dx / 2, 1 + dx / 2)),
           file.begin(), file.end(), leafSize);

   std::cout << "Memory Usage = " << orq.getMemoryUsage() << "\n";

   // Container for records found in a window query.
   RecordContainer closePoints;
   // The query window.
   BBox window;
   Timer timer;
...
exitOnError() {
   std::cerr
         << "Bad arguments.  Usage:\n"
         << programName << " [-leafSize=n] <grid size> <search radius>\n";
   exit(1);
}
