...
main(int argc, char* argv[]) {
   ...
   typedef geom::Placebo<3, Record> Orq;
   // Bounding box.
   ...
   // The query size.
   int querySize = -1;
   if (! parser.getArgument(&querySize)) {
      std::cerr << "Error in reading the query size.\n";
      exitOnError();
   }
   if (querySize < 0) {
      std::cerr << "Bad value for the query size: " << querySize << "\n";
      exitOnError();
   }

   std::cout << "Query size = " << querySize << "\n";
...
exitOnError() {
   std::cerr
         << "Bad arguments.  Usage:\n"
         << programName << " <mesh> <search radius> <query size>\n";
   exit(1);
}
