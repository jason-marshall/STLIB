...
#include "geom/orq/CellForwardSearchKey.h"

#include "ads/functor/coordinateCompare.h"
#include "ads/timer.h"
...
main(int argc, char* argv[]) {
   ...
   typedef geom::CellForwardSearchKey<3, Record> Orq;
   // Bounding box.
   ...
   // Sort the points in the z direction.  This ensures that the window
   // queries are performed in the correct order.
   std::sort(points.begin(), points.end(),
             ads::constructLessThanCompareCoordinate<Point>());
...
exitOnError() {
   std::cerr
         << "Bad arguments.  Usage:\n"
         << programName << " <mesh> <search radius> <dx> <dy>\n";
   exit(1);
}
