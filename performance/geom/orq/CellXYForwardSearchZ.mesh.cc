...
#include "CellXYForwardSearchZ.h"
#include "Record.h"
...
main(int argc, char* argv[]) {
   ...
   typedef geom::CellXYForwardSearchZ<Record> CellXYForwardSearchZ;
   typedef CellXYForwardSearchZ::bbox_type BBox;
   typedef CellXYForwardSearchZ::semi_open_interval_type SemiOpenInterval;
   ...
   std::sort(records.begin(), records.end(), geom::zless<Record>());

   // Construct
   CellXYForwardSearchZ cfs(Point(dx, dy, dy), domain, records.begin(),
                            records.end());

   std::cout << "memory usage = " << cfs.memory_usage() << '\n';

   CellXYForwardSearchZ::value_type* close_pts
   = new CellXYForwardSearchZ::value_type[cfs.num_records()];
   ...
   for (int i = 0; i < num_vertices; ++i) {
      ...
      count += cfs.window_query(close_pts, window);
   }
