...
main(int argc, char* argv[]) {
   ...
   //
   // Make a vector of random doubles.
   //
   vector< double > v(length);
   std::subtractive_rng random;
   typename vector<double>::iterator i = v.begin();
   typename vector<double>::const_iterator i_end = v.end();
   for (; i != i_end; ++i)
      *i = double(random(1000)) / 1000;

   double val = double(random(1000)) / 1000;
