...
main(int argc, char* argv[]) {
   ...
   typedef geom::SequentialScan<Record> SequentialScan;
   typedef SequentialScan::bbox_type BBox;
   ...
   // Construct
   SequentialScan ss(records.begin(), records.end());

   SequentialScan::value_type* close_pts
   = new SequentialScan::value_type[ss.num_records()];

   std::vector<int> count(num_tests, 0);
   double elapsed_time;
   Timer timer;
   int n = 0;
   ...
   // Warm up.
   for (int j = 0; j < num_queries_per_test; ++j) {
      ss.window_query(close_pts, queries[j]);
   }

   for (int i = 0; i < num_tests; ++i) {
      timer.tic();
      for (int j = 0; j < num_queries_per_test; ++j) {
         count[i] += ss.window_query(close_pts, queries[n]);
         ++n;
      }
   ...
   // Write the average number of reported records per query.
   for (int i = 0; i < num_tests; ++i) {
      std::cout << (1.0 * count[i]) / num_queries_per_test << "," << '\n';
   }
   std::cout << '\n';
