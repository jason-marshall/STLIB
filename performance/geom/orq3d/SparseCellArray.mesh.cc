...
main(int argc, char* argv[]) {
   ...
   typedef geom::SparseCellArray<Record> SparseCellArray;
   typedef SparseCellArray::bbox_type BBox;
   typedef SparseCellArray::semi_open_interval_type SemiOpenInterval;

   // Check for bad arguments.
   if (argc != 6) {
      std::cerr << "Bad arguments." << '\n'
                << "Usage: SparseCellArray mesh.ascii dx dy dz search_radius"
                << '\n';
      exit(1);
   }
   ...
   SparseCellArray sca(Point(dx, dy, dz), domain, records.begin(),
                       records.end());

   std::cout << "memory usage = " << sca.memory_usage() << '\n';

   SparseCellArray::value_type* close_pts
   = new SparseCellArray::value_type[sca.num_records()];
   ...
   for (int i = 0; i < num_vertices; ++i) {
      ...
      count += sca.window_query(close_pts, window);
   }
