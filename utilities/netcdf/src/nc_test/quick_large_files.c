 ...
 *   $Id: quick_large_files.c 411 2006-03-14 23:06:58Z sean $
 *********************************************************************/
...
/* These are handy for constucting tests. */
#define QTR_CLASSIC_MAX (MAX_CLASSIC_BYTES/4)
#define QTR_64OFFSET_MAX (MAX_64OFFSET_BYTES-2/4)
...
main(int argc, char **argv)
{
	...
	{
	    cflag |= NC_SHARE;
	    printf("turning on NC_SHARE for subsequent test...\n");
	}

	/* Create a netCDF 64-bit offset format file. Write a value. */
	printf("Creating %s for 64-bit offset large file test...\n", file);
