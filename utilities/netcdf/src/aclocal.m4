dnl $Id: aclocal.m4 411 2006-03-14 23:06:58Z sean $
...
ud_which_cc=`which $CC`
AC_MSG_CHECKING(which $CC)
AC_MSG_RESULT($ud_which_cc)
...
if test -n "$CXX"; then
	...
	ud_which_CXX=`which $CXX`
	AC_MSG_CHECKING(which $CXX)
	AC_MSG_RESULT($ud_which_CXX)
else
	AC_MSG_RESULT(unset)
fi
...
if test -n "$FC"; then
	...
	ud_which_fc=`which $FC`
	AC_MSG_CHECKING(which $FC)
	AC_MSG_RESULT($ud_which_fc)
else
	AC_MSG_RESULT(unset)
fi
...
if test -n "$F90"; then
	...
	AC_MSG_RESULT($F90FLAGS)
	ud_which_F90=`which $F90`
	AC_MSG_CHECKING(which $F90)
	AC_MSG_RESULT($ud_which_F90)
else
	AC_MSG_RESULT(unset)
fi
...
if test -n "$AR"; then
	...
	AC_MSG_RESULT($ARFLAGS)
	ud_which_AR=`which $AR`
	AC_MSG_CHECKING(which $AR)
	AC_MSG_RESULT($ud_which_AR)
else
	AC_MSG_RESULT(unset)
fi
...
if test -n "$NM"; then
	...
	AC_MSG_RESULT($NMFLAGS)
	ud_which_NM=`which $NM`
	AC_MSG_CHECKING(which $NM)
	AC_MSG_RESULT($ud_which_NM)
else
	AC_MSG_RESULT(unset)
fi
...
if test -n "$RANLIB"; then
	...
	AC_MSG_CHECKING(RANLIBFLAGS)
	AC_MSG_RESULT($RANLIBFLAGS)
	ud_which_RANLIB=`which $RANLIB`
	AC_MSG_CHECKING(which $RANLIB)
	AC_MSG_RESULT($ud_which_RANLIB)
else
	AC_MSG_RESULT(unset)
fi
