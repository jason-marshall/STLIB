...
#if !defined(__performance_numerical_interpolation_PolynomialInterpolationUsingCoefficients_ipp__)
#error This file is an implementation detail of PolynomialInterpolationUsingCoefficients.
#endif
...
main(int argc, char* argv[]) {
   ...
   numerical::PolynomialInterpolationUsingCoefficients<double, Order> 
      f(functionValues.begin(), functionValues.size(), 0., numberOfCells);
...
#if defined(PERFORMANCE_SECOND_DERIVATIVE)
   double first, second;
#elif defined(PERFORMANCE_FIRST_DERIVATIVE)
   double first;
#endif
   ads::Timer timer;
   timer.tic();
...
#if defined(PERFORMANCE_SECOND_DERIVATIVE)
         meaninglessResult += f(x[j], &first, &second);
         meaninglessResult += first + second;
#elif defined(PERFORMANCE_FIRST_DERIVATIVE)
         meaninglessResult += f(x[j], &first);
         meaninglessResult += first;
#else
