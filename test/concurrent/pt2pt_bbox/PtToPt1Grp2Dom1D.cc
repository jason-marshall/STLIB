...
#include "concurrent/pt2pt_bbox/PtToPt1Grp2Dom.h"
...
main(int argc, char* argv[]) {
   ...
   typedef concurrent::PtToPt1Grp2Dom<1>::bbox_type bbox_type;
   ...
   // called before MPI::Finalize().
   {
      ...
      bbox_type data_domain, interest_domain;
#ifdef PT2PT_BBOX_USE_CPP_INTERFACE
      concurrent::PtToPt1Grp2Dom<1> p2p(MPI::COMM_WORLD);
#else
      concurrent::PtToPt1Grp2Dom<1> p2p(MPI_COMM_WORLD);
#endif
      std::vector<bbox_type> overlap_interest_domains, overlap_data_domains;
      std::vector<int> send_ranks, receive_ranks;
      ...
      for (; num_iterations; --num_iterations) {
         ...
         interest_domain =
            bbox_type(data_domain.getLowerCorner() - 0.1 *
                      (data_domain.getUpperCorner() -
                       data_domain.getLowerCorner()),
                      data_domain.getUpperCorner() + 0.1 *
                      (data_domain.getUpperCorner() -
                       data_domain.getLowerCorner()));
#if USE_MPE
         ...
         p2p.solve(data_domain, interest_domain, comm_world_rank,
                   ...
                   std::back_inserter(send_ranks),
                   std::back_inserter(overlap_data_domains),
                   std::back_inserter(receive_ranks));
#if USE_MPE
...
#endif
         included_records_array.resize(send_ranks.size());
         for (std::size_t i = 0, i_end = send_ranks.size(); i != i_end; ++i) {
            std::vector<point_type>& included_records = included_records_array[i];
            included_records.clear();
            const bbox_type their_interest_domain = overlap_interest_domains[i];
            const int their_rank = send_ranks[i];
            ...
            for (; record_iter != records_end; ++record_iter) {
               ...
               if (their_interest_domain.isIn(*record_iter)) {
            ...
            MPI_Isend(&*included_records.begin(),
                      included_records.size() * sizeof(point_type),
                      MPI::BYTE, their_rank, TAG_RECORDS, MPI_COMM_WORLD,
                      &request);
         ...
         //
         recv_records.clear();
         for (int i = 0, i_end = receive_ranks.size(); i != i_end; ++i) {
#ifdef PT2PT_BBOX_USE_CPP_INTERFACE
            MPI::Status status;
      ...
      //
      std::size_t num_comm = recv_records.size();
      int sum = 0;
#ifdef PT2PT_BBOX_USE_CPP_INTERFACE
...
#if USE_MPE
   MPE_Finish_log("PtToPt1Grp2Dom1D");
#endif
...
exit_on_error(const char* error_message) {
   std::cerr << error_message
             << "\n\nUsage:\n"
             << "PtToPt1Grp2Dom1D.exe num_records num_iterations\n"
