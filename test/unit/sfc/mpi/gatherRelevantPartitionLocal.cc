...
main(int argc, char* argv[])
{
  ...
  // Declare that only the local cells are relevant.
  std::vector<std::size_t> relevantCells;
  {
    std::vector<Code> relevantCodes(localCells.size());
    for (std::size_t i = 0; i != relevantCodes.size(); ++i) {
      relevantCodes[i] = localCells.code(i);
    }
    relevantCells = distributedCells.codesToCells(relevantCodes);
  }
  ...
  // Gather the relevant objects with the ring pattern.
  {
    ...
    std::size_t const total =
      stlib::mpi::reduce(relevantObjects.size(), MPI_SUM);
    if (commRank == 0) {
      assert(total >= numDistributedObjects);
    }
  }
