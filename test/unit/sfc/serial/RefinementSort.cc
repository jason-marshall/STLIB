...
main()
{
  // 1-D, 0 levels
  {
    ...
    typedef sfc::RefinementSort<Traits>::Pair Pair;

    BlockCode blockCode(Point{{0}}, Point{{1}}, 0);
    {
      std::vector<Pair> pairs;
      sfc::refinementSort(blockCode, &pairs, 0);
      assert(pairs.empty());
    }
    {
      ...
      sfc::refinementSort(blockCode, &p, 1);
      assert(p == pairs);
    }
  }
  // 1-D, 1 level
  {
    ...
    // Put all of the points at the origin.
    {
      ...
      sfc::refinementSort(blockCode, &p, 1);
      assert(p == pairs);
      sfc::refinementSort(blockCode, &p, p.size());
  ...
  // 3-D, 4 levels
  {
    ...
    BlockCode blockCode(Point{{0, 0, 0}}, Point{{1, 1, 1}}, 4);
    {
      ...
      for (std::size_t n = 0; n <= 4; ++n) {
        ...
        sfc::refinementSort(blockCode, &p, maxElementsPerCell);
        assert(p.size() == pairs.size());
#if 0
    ...
    // Sort objects.
    {
      ...
      }
      {
        ...
        sfc::refinementSort(blockCode, &obj, &codeIndexPairs, 1);
