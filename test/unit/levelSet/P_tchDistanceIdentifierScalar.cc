...
main()
{
  ...
  assert(patch.identifiers[0] == std::numeric_limits<unsigned>::max());
  assert(patch.identifiers[Patch::NumPoints - 1] ==
  ...
  assert(patch.identifiers[0] == 0);

  patch.conditionalSetValueGe(0.f, -std::numeric_limits<float>::infinity());
  for (std::size_t i = 0; i != Patch::NumPoints; ++i) {
    assert(patch[i] < 0);
    if (patch[i] == -std::numeric_limits<float>::infinity()) {
      assert(patch.identifiers[i] == std::numeric_limits<unsigned>::max());
    }
    else {
      assert(patch.identifiers[i] == 0);
    }
  }
