...
test()
{
  ...
  typedef geom::CellArray<3, ads::Dereference<Record> > ORQDS;
...

  {
    ...
    ORQDS x(Point{{0.5, 0.34, 0.25}},
            BBox{{{0., 0., 0.}},
              {{1., 1., 1.}}});
    std::cout << "CellArray((0.5,0.34,0.25), (0,0,0,1,1,1)) = \n"
              << x << '\n';
  }
  {
    ValueContainer v;
    ...
    {
      // Construct from the cell size, a domain, and a list of records.
      ORQDS x(Point{{0.5, 0.34, 0.25}},
              BBox{{{0., 0., 0.}},
                {{1., 1., 1.}}},
              v.begin(), v.end());
    ...
    {
      // Construct from the cell size and a list of records.
    ...
    {
      // Construct from the number of cells and a list of records.
      ORQDS x(27, v.begin(), v.end());
      std::cout << "Use the number of cells and a range of initial values.\n"
                << x << '\n';
      x.clear();
      assert(x.size() == 0);
    }
...

  {
    ...
    ValueContainer v;
    for (std::size_t i = 0; i != 8; ++i) {
      v.push_back(Value{{_Float(random()),
              _Float(random()),
              _Float(random())}});
    }
    ORQDS x(Point{{0.5, 0.34, 0.25}},
...

  {
    ...
    ValueContainer v;
    for (std::size_t i = 0; i != 8; ++i) {
      v.push_back(Value{{_Float(1 + random()),
              _Float(2 + random()),
              _Float(3 + 2 * random())}});
    }
    x.insert(v.begin(), v.end());
