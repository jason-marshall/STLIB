...
#if !defined(__geom_CellForwardSearch_ipp__)
#error This file is an implementation detail of the class CellForwardSearch.
#endif
                   ...
                   const typename Base::BBox& window) const
{
  ...
  std::size_t count = 0;
  typename Base::Cell::const_iterator recordIterator, recordIteratorEnd;
  ...
  const typename Base::Float coord_min = window.lower[N - 1];
  const typename Base::Float coord_max = window.upper[N - 1];
  std::size_t n = N - 2;
  mi = lo;
  while (mi[N - 2] <= hi[N - 2]) {
    if (n == 0) {
      for (mi[0] = lo[0]; mi[0] <= hi[0]; ++mi[0]) {
        ...
        if (isInside(interior, indexPoint)) {
          for (; recordIterator != recordIteratorEnd &&
               Base::_location(*recordIterator)[N - 1] <= coord_max;
               ++recordIterator) {
