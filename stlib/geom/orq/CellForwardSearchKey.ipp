...
inline
std::size_t
CellForwardSearchKey<N, _Location>::
                   ...
                   const typename Base::BBox& window) const
{
  ...
  typename Base::Cell::KeyConstIterator keyIterator, keyIteratorEnd;
    ...
    if (n == 0) {
      for (mi[0] = lo[0]; mi[0] <= hi[0]; ++mi[0]) {
        ...
        if (isInside(interior, indexPoint)) {
          ...
          for (; keyIterator != keyIteratorEnd && *keyIterator <= coordMax;
