...
public:
  ...
  isValid(const Lattice& lattice, Number maximumDistance,
          ...
          std::size_t maximumReportedErrors = 1000) const;
    ...
    for (MultiIndexRangeIterator i = MultiIndexRangeIterator::begin(getRanges());
      ...
      if (gd == HugePoint) {
        ...
        if (isClosestPointBeingComputed()) {
          ...
          cp = getClosestPoint()(*i);
          if (ext::magnitude(ext::cross(position - cp, gd)) >
              std::sqrt(std::numeric_limits<Number>::epsilon())) {
            std::cerr << "In Grid<3,T>::isValid():\n"
                      << "    Direction of gradient is wrong.\n"
                      << "    gradient = " << gd << '\n'
                      << "    position - closest point = " << position - cp
                      << '\n'
                      << "    (i,j,k) = " << *i << '\n';
            result = false;
            if (++numberOfErrors >= maximumReportedErrors) {
...
floodFill(const Number farAway)
{
  ...
  bool result = false;
  int sign = 0;
  typename container::MultiArray<Number, 3>::const_iterator iter =
    getDistance().begin();
  const typename container::MultiArray<Number, 3>::const_iterator iterEnd =
    getDistance().end();
