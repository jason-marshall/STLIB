...
#if !defined(__levelSet_PatchColMajDistance_ipp__)
#error This file is an implementation detail of PatchColMajDistance.
#endif
...
{
  _lowerCorner = lowerCorner;
  std::fill(grid.begin(), grid.end(), _mm256_set1_ps(value));
}
  ...
  for (std::size_t i = 0; i != NumVectors; ++i) {
    grid[i] =
      _mm256_min_ps(grid[i],
                    _mm256_sqrt_ps(_dx + _dy[i & 0x7] + _dz[i >> 3]) - r);
  }
}
...
unionEuclidean(const Ball& ball)
{
  ...
  for (std::size_t i = 0; i != NumVectors; ++i) {
    grid[i] =
      _mm_min_ps(grid[i],
                 _mm_sqrt_ps(_dx[i & 0x1] + _dy[(i >> 1) & 0x7] +
                             _dz[i >> 4]) - r);
  }
}
