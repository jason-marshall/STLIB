...
#if !defined(__hj_EikonalAdjDiag2nd2_ipp__)
...
template<typename T>
inline
typename EikonalAdjDiag2nd<2, T>::Number
EikonalAdjDiag2nd<2, T>::
...
diff_diag(const IndexList& i, const IndexList& d) const {
   ...
   if (_status(i2, j2) == KNOWN &&
         _solution(i1, j1) > _solution(i2, j2)) {
      return diff_d2(_solution(i1, j1), _solution(i2, j2), _inverseSpeed(i));
   }
   return diff_d1(_solution(i1, j1), _inverseSpeed(i));
}
