...
#if !defined(__hj_Eikonal2_ipp__)
...
template<typename T>
inline
typename Eikonal<2, T>::Number
Eikonal<2, T>::
diff_a2(const Number a1, const Number a2, const Number f) const {
   return (4 * a1 - a2 + 2 * _dx * f) / 3;
}
...
typename Eikonal<2, T>::Number
Eikonal<2, T>::
diff_d2(const Number d1, const Number d2, const Number f) const {
   return (4 * d1 - d2 + 2 * _dx_t_sqrt2 * f) / 3;
}
...
typename Eikonal<2, T>::Number
Eikonal<2, T>::
diff_a1_a1(const Number a, const Number b, const Number f) const {
   ...
   // difference scheme will not be upwind.  For this case, we return infinity.
   if (std::abs(a - b) <= _dx * f) {
      return 0.5 *(a + b + std::sqrt(2 * _dx2 *(f*f) - (a - b) *(a - b)));
   }
...
typename Eikonal<2, T>::Number
Eikonal<2, T>::
diff_a1_d1(const Number a, const Number d, const Number f) const {
   // If the following condition is not satisfied, then the characteristic
   ...
   Number diff = a - d;
   if (0 <= diff && diff <= _dx_o_sqrt2 * f) {
      return a + std::sqrt(_dx2 *(f * f) - diff * diff);
   }
   return std::numeric_limits<Number>::max();
}
...
typename Eikonal<2, T>::Number
Eikonal<2, T>::
diff_a2_a1(const Number a1, const Number a2, const Number b,
           const Number f) const {
   Number disc = 13 * _dx2 * (f * f) - (4 * a1 - a2 - 3 * b) *
                 (4 * a1 - a2 - 3 * b);
   if (disc >= 0) {
...
typename Eikonal<2, T>::Number
Eikonal<2, T>::
diff_a1_d2(const Number a, const Number d1, const Number d2,
           const Number f) const {
   const Number t = 3 * a - 4 * d1 + d2;
   const Number disc = 5 * _dx2 * (f * f) - t * t;
   if (disc >= 0) {
...
typename Eikonal<2, T>::Number
Eikonal<2, T>::
diff_a2_d1(const Number a1, const Number a2, const Number d,
           const Number f) const {
   const Number t = 4 * a1 - a2 - 3 * d;
   const Number disc = 10 * _dx2 * (f * f) - t * t;
   if (disc >= 0) {
...
diff_a2_a2(const Number a1, const Number a2,
           const Number b1, const Number b2, const Number f) const {
   Number disc = 8 * _dx2 * (f * f)
...
diff_a2_d2(const Number a1, const Number a2,
           const Number d1, const Number d2, const Number f) const {
   const Number t = 4 * a1 - a2 - 4 * d1 + d2;
   const Number disc = 4 * _dx2 * (f * f) - t * t;
   if (disc >= 0) {
