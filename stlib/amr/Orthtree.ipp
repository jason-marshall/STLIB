                   ...
                   iterator finish)
{
  ...
  for (iterator i = start; i != finish; ++i) {
    ...
    if (hasParent(i->first) && isLowerCorner(i->first)) {
      ...
      for (std::size_t n = 1; n != NumberOfOrthants && isMergeable;
           ++n, ++j) {
...
findAncestor(const SpatialIndex& key) const
{
  ...
  // If the node exists, the search will find it. Otherwise if any of its
...
linkBalanced(iterator node)
{
  ...
  for (std::size_t d = 0; d != 2 * _Traits::Dimension; ++d) {
    ...
    if (hasNeighbor(spatialIndex, d)) {
      ...
      iterator ancestor = findAncestor(spatialIndex);
      if (ancestor == end()) {
        node->second.adjacent[d] = 0;
      }
      else {
        node->second.adjacent[d] = &*ancestor;
      }
    }
...
synchronizeBalanced(iterator node)
{
  ...
  const _Patch* source;
  std::array < const _Patch*, _Traits::NumberOfOrthants / 2 > neighbors;
  // For each signed direction.
  for (std::size_t d = 0; d != 2 * _Traits::Dimension; ++d) {
    const value_type* neighbor = node->second.adjacent[d];
    if (neighbor != 0) {
...
#ifdef STLIB_DEBUG
        assert((*neighbors)[i]->adjacent[2 * d + 1] != 0);
#endif
        (*neighbors)[size + i] =
          &((*neighbors)[i]->adjacent[2 * d + 1]->second);
      }
      size *= 2;
    }
  }
