...
/*!
  \file inside2.cc
  \brief Determine which grid points are inside the 2-D solid.
*/

/*!
  \page cpt_inside2 Determine which grid points are inside the 2-D solid.


  \section cpt_driver2_introduction  Introduction

  The file <tt>stlib/examples/cpt/inside2.cc</tt> shows how to call the
  functions that determine which grid points are inside the sollid.
  Below are the command line arguments.

  \verbatim
  ./inside2.exe geom brep out
  ...
  \section cpt_inside2_summary Summary
  ...
  \section cpt_input2_compiling Compiling.

  Use gnu "make" on "Makefile" with the command "make inside2.exe" or
  ...
  \section cpt_driver2_output_file_format Output File Format
